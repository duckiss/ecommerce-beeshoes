...
public class SendEmailService {
    ...
    public void sendEmailRollBackBill(String to, String note,  String idBill) {
        ...
        RollBackBillResponse response = RollBackBillResponse.builder().code(optional.get().getCode()).fullName(optional.get().getEmployees().getUser().getFullName()).note(note).url("http://beeshoes-frontend-service:80//bill-management/detail-bill/"+ optional.get().getId()).build();
        String htmlBody = "<html>"
                + "<head>"
                + "<style>"
                + "body { font-family: Arial, sans-serif; background-color: #007bff; color: #ffffff; padding: 20px; }"
